...
filename = '../util/proj1_3_world.json'
...
ax.legend(handles=[
    Line2D([], [], color='red', linewidth=1, label='Dense A* Path (' + str(my_world_traj.n_pts) + ')'),
    Line2D([], [], color='purple', linestyle='', marker='.', markersize=8, label='Sparse Waypoints ('+
                                                                                 str(my_world_traj.new_pts) + ')'),
...
fig.savefig('../data_out/Plot_output_elev_' + str(elev) + '_axim_' + str(azim) + '.png')
